...
Diff.l(one,'ligno') = Peak_Area*exp(-1*(LANDPRICE.l(one)-200)/200)/(1+exp(-alpha*(time-T_zero)));
...
*     +
      [
       -Diff(one,biomass) + Peak_Area*exp(-1*(LANDPRICE(one)-200)/200)/(1+exp(-alpha*(time-T_zero)))
      ]$(sameas(biomass,'ligno') and LP_feedback )
      +
      [
       -Diff(one,biomass) + Peak_Area/(1+exp(-alpha*(time-T_zero)))
      ]$(sameas(biomass,'ligno') and  not LP_feedback )
      +
      [
       -Diff(one,biomass) + 1
      ]$(sameas(biomass,'crop') or sameas(biomass,'grass') and competition)
...
*       -Diff(one,biomass) + (1-(ALAREA(one,'ligno')-base_area('ligno'))*beta(one,biomass)/base_area(biomass))
      ...
      ]$(sameas(biomass,'crop') or sameas(biomass,'grass') and not competition)
